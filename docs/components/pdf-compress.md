...
## 役割
ブラウザから presigned POST でアップロードしたPDFを圧縮し、複数候補（高画質/標準/高圧縮）を返すサービス。Ghostscriptを使用。
