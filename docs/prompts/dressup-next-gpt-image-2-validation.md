...
## high-risk fit v13: clothing

`ITEM_BATCH=high-risk-fit-v13-clothing` では、服カテゴリの成立性を確認します。生成対象は top 1点、bottom 1点、dressOverlay 1点の合計 3 call に限定します。服は腕・肩・襟・腰・base dress の隠れ方に影響するため、単体合成で破綻しないかを先に確認します。

実行:

```bash
ITEM_BATCH=high-risk-fit-v13-clothing npm --prefix backend/draw run generate-dressup-gpt-image-2-validation
```

特定アイテムだけ再生成する場合:

```bash
ITEM_BATCH=high-risk-fit-v13-clothing ITEM_IDS=top-frill-blouse-fit npm --prefix backend/draw run generate-dressup-gpt-image-2-validation
ITEM_BATCH=high-risk-fit-v13-clothing ITEM_IDS=bottom-fluffy-skirt-fit npm --prefix backend/draw run generate-dressup-gpt-image-2-validation
ITEM_BATCH=high-risk-fit-v13-clothing ITEM_IDS=dress-overlay-pastel-fit npm --prefix backend/draw run generate-dressup-gpt-image-2-validation
```

v13 の配置方針:

- base cutout から body center X、upper body bounds、skirt bounds を測定する。
- top は肩・首元・胴体中心へ合わせ、腕を覆いすぎないかを見る。
- bottom は腰位置へ合わせ、base dress の裾を自然に隠せるかを見る。
- dressOverlay は肩からスカート裾までの一体 layer として、top/bottom 分割より自然かを見る。
- 初回はアクセサリとの組み合わせは行わず、base model への単体合成だけを確認する。

出力:

- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/raw/`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/cutout/`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/normalized/`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/composite/`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/item-fit-v13-preview.html`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v13-clothing/item-fit-v13-review.md`

v13 で top / bottom / dressOverlay のうち、分割服が不自然で一体 dressOverlay が自然な場合は、服カテゴリを `dressOverlay` 中心にする判断が必要です。逆に top と bottom が自然に重なる場合は、既存ゲーム仕様に近い `top` / `bottom` 分割を維持できます。
...
## full outfit v15: layer stack

`ITEM_BATCH=full-outfit-v15-layer-stack` では、ゲーム実装へ進む前に、生成済み normalized layer だけを使って全身コーデの上下関係を確認します。新規画像生成は行わず、生成 call は 0 です。

実行:

```bash
RENDER_ONLY=1 ITEM_BATCH=full-outfit-v15-layer-stack npm --prefix backend/draw run generate-dressup-gpt-image-2-validation
```

v15 の比較方針:

- top は v13 `top-frill-blouse-fit` を使う。
- bottom は v14 `bottom-long-frill-skirt-fit` と `bottom-knee-a-line-fit` の2種を比較する。
- boots は v10 opaque boots を使う。
- necklace は v8 re-anchored baseline を使う。
- hairAccessory は v12 side ribbon hairpin を使う。
- レイヤー順は `top -> bottom`、`bottom -> top`、`bottom -> top -> necklace before boots` の3パターンを比較する。

出力:

- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v15-layer-stack/composite/`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v15-layer-stack/item-fit-v15-preview.html`
- `backend/draw/artifacts/dressup-gpt-image-2-validation/item-fit-v15-layer-stack/item-fit-v15-review.md`

v15 で1つ以上の自然なレイヤー順が見つかれば、次は最小 PNG 版ゲーム実装に進みます。ゲーム実装へ進む場合は、ルート、公開方針、既存 `/games/dressup/` との関係が設計決定になるため、`docs/architecture.yaml` 更新対象として扱います。
