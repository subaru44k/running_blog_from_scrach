...
- `/games/dressup-next/` は PNG レイヤー版おしゃれゲームの公開ルート。既存 `/games/dressup/` は置き換えず、各部位に画像プレビュー付きの約10種類の選択肢を持ち、完成後は編集用ホットスポットを隠し、チャットさんが部位ごとに選ぶ様子を表示してから「どっちがすき？」を選んで遊ぶ。sitemap と games Service Worker のナビゲーション/画像キャッシュ対象に含める。
