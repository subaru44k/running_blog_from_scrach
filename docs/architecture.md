...
- `/games/` は軽量なミニゲーム集のハブ。`/games/balloon-catch/`、`/games/dressup/`、`/games/match-quiz/`、`/games/janken/`、`/games/clock/`、`/games/snake/`、`/games/maze/`、`/games/tic-tac-toe/`、`/games/reversi/` を含む。
...
- `/games/dressup-next/` は PNG レイヤー版おしゃれゲームの検証ルート。既存 `/games/dressup/` は置き換えず、`noindex,follow`、sitemap 除外、games Service Worker のナビゲーション/画像キャッシュ対象外として扱う。
...
- `/games/` 系は専用の Service Worker により静かにオフライン対応する。訪問後に `/games/` と各ゲームページ、および必要な静的アセットをキャッシュし、他のルートや API には作用させない。ただし検証用の `/games/dressup-next/` とその PNG アセットはキャッシュ対象外。
