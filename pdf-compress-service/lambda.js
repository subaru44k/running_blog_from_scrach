...
exports.handler = async (event) => {
  try {
    ...
    let fileBuf, filename, level, removeMetadata, grayscale;
    ...
    if (/multipart\/form-data/i.test(contentType)) {
      ...
      // Expect JSON: { fileBase64, filename?, level?, removeMetadata?, grayscale? }
      ...
      if (!data.fileBase64) throw new Error('Missing fileBase64 in JSON body');
