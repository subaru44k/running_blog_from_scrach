...
type GamepadControllerOptions = {
  ...
  onPrimary?: () => void;
  onSecondary?: () => void;
  directionRepeatMs?: number | null;
  axisThreshold?: number;
};

const DIRECTION_BUTTONS: Record<GamepadDirection, number> = {
  up: 12,
  down: 13,
  left: 14,
  right: 15,
...
function isAnyButtonPressed(gamepad: Gamepad, indices: number[]) {
  return indices.some((index) => Boolean(gamepad.buttons[index]?.pressed));
}

function readDirection(gamepad: Gamepad, axisThreshold: number): GamepadDirection | null {
...
export function createGamepadController({
  ...
  let rafId = 0;
  let previousDirection: GamepadDirection | null = null;
